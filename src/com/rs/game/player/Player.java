...
public class Player extends Entity {
	...
	public void run() {
		...
		getPackets().sendGameMessage("Welcome to <col=9BBC74>"+Settings.SERVER_NAME+ "</col>, " + getUsername() + "!");
		...
		getInterfaceManager().openGameTab(4);
		...
		if (getClassName() == "" || getClassName() == null) {
			getPackets().sendGameMessage("You're new to <col=9BBC74>"+Settings.SERVER_NAME+ "</col>? Speak to the Eliwen Guide!");
			getHintIconsManager().addHintIcon(World.getNpc(1182), 0, -1, false);
		}
		
        //welcome message opengl
        //PlayerLook.openThessaliasMakeOver(this);
		//signUp();
		getTools().refresh();
	...
	public void realFinish() {
		...
		if (Settings.DEBUG) {
			Logger.log(this, "Finished Player: " + username + ", pass: " + purePassword);
			Logger.log(this, getRights());
		}
		if (getRights() == 2) {
		}else if (getRights() == 10) {
		}else{
	...
	private boolean hasDiedBefore;

	public boolean hasDiedBefore() {
		return hasDiedBefore;
	}

	public void sethasDiedBefore(boolean x) {
		this.hasDiedBefore = x;
	}
	...
	public boolean isOwner() {
		return username.toLowerCase().equals(Settings.OWNERS[0]);
	}
    ...
    // welcome message opengl warning
	...
	public void signUp() {
        ...
        */
		...
		PlayerLook.openCharacterCustomizing(this);
		getPackets().sendIComponentText(900, 33, "Confirm");
		getPackets().sendConfigByFile(6098, getAppearence().isMale() ? 0 : 1);
		getPackets().sendConfigByFile(6099, getAppearence().getSkinColor());
		getAttributes().put("MageMakeOverGender", getAppearence().isMale());
		getAttributes().put("MageMakeOverSkin", getAppearence().getSkinColor());
	}
