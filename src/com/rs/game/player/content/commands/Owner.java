...
public class Owner {
	...
	public static boolean processCommand(Player player, String[] cmd, boolean console, boolean clientCommand) {
		...
		if (cmd[0].equals("pos")) {
			player.getPackets().sendGameMessage("X: " + player.getX() + " | Y: " + player.getY());
			return true;
        }
		...
		if (cmd[0].equals("givepackage")) {
			int i = Integer.parseInt(cmd[1]);
			DonationManager.getReward(player,i);
			player.sendMessage(player.getDisplayName()+", you've been given donar package "+i+"!");
		}
        ...
        */
		...
		if (cmd[0].equals("setskill")) {
			...
			player.getSkills().set(skillId, 99);
			player.getSkills().setXp(skillId, Skills.getXPForLevel(level));
			if (level == 120)
				player.getSkills().setXp(skillId, 104273167);
			player.sendMessage("Your "+Skills.SKILLS[skillId]+" is now level "+level+"!");
			return true;
		}

		if (cmd[0].equals("all120")) {
			for (int i = 0; i < 25; i++) {
					player.getSkills().set(i, 99);
					player.getSkills().setXp(i, 104273167);
			}
			return true;
		}

		if (cmd[0].equals("resetallskills")) {
			for (int i = 0; i < 25; i++) {
		...
		if (cmd[0].equals("setvp")) {
			...
			player.getPackets().sendGameMessage("<col=B51749>You've set "+target.getDisplayName()+"'s vote points has been set to "+vp+"</col>");
			SerializableFilesManager.savePlayer(target);
			if (World.isOnline(name)) {
				target.getPackets().sendGameMessage("Your vote points have been set to<col=B51749> "+vp+"</col> by <img=10>"+player.getDisplayName()+"");
			}
			return true;
		}
		...
		if (cmd[0].equals("setpp")) {
			...
			player.getPackets().sendGameMessage("<col=B51749>You've set "+target.getDisplayName()+"'s prestige points has been set to "+vp+"</col>");
			SerializableFilesManager.savePlayer(target);
			
			if (World.isOnline(name)) {
				target.getPackets().sendGameMessage("Your prestige points has been set to <col=B51749>"+vp+"</col> by <img=10>"+player.getDisplayName()+"!");
			}
			return true;
		}
		...
		if (cmd[0].equals("setprestige")) {
			...
			SerializableFilesManager.savePlayer(target);
			if (World.isOnline(name)) {
				target.getPackets().sendGameMessage("<img=10>Your prestige level has been set to "+vp+" by "+player.getDisplayName()+"");
			}
			return true;
		}
		...
		if (cmd[0].equals("addobj")) {
			...
			int id = Integer.parseInt(cmd[1]);
			//int face = cmd.length < 3 ? 0 : Integer.parseInaddt(cmd[2]);
            int face = 1;
			int diffX = cmd.length < 4 ? 0 : Integer.parseInt(cmd[3]);
			int diffY = cmd.length < 5 ? 0 : Integer.parseInt(cmd[4]);
            if (cmd[2].equalsIgnoreCase("north"))
                face = 0;
            if (cmd[2].equalsIgnoreCase("east"))
                face = 1;
            if (cmd[2].equalsIgnoreCase("south"))
                face = 2;
            if (cmd[2].equalsIgnoreCase("west"))
                face = 3;
			ObjectSpawning.writeObject(player, id, player.getX() + diffX, player.getY() + diffY, player.getPlane(), face, true);
			World.spawnObject(new WorldObject(id, 10, face, player.getX() + diffX, player.getY() + diffY, player.getPlane()), true);
			return true;
		}
		...
		if (cmd[0].equals("removeobj")) {
            ...
            if (cmd[2].equalsIgnoreCase("west"))
                face = 3;
			ObjectSpawning.writeObject(player, id, player.getX() + diffX, player.getY() + diffY, player.getPlane(), face, true);
			World.removeObject(new WorldObject(id, 10, face, player.getX() + diffX, player.getY() + diffY, player.getPlane()), true);
			return true;
		}
